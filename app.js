...
window.printBid=function(id){
  ...
  el("invoiceArea").innerHTML=`<div class="invoice"><div class="invoiceTop"><div><img class="invoiceLogo" src="logo.png" alt="${COMPANY.name}" onerror="this.style.display='none'"><h2>${COMPANY.name}</h2><p>${COMPANY.tagline}</p><p>${COMPANY.phone}</p><p>${COMPANY.email}</p></div><div><h1>Proposal</h1><p><b>${safe(b.title)}</b></p><p>Date: ${dateLabel(today())}</p><span class="badge ${b.status==="Approved"?"badgeGreen":"badgeBlue"}">${safe(b.status||"Pending")}</span></div></div>${c?`<h3>Prepared For</h3><p><b>${safe(c.name)}</b><br>${safe(c.email)}<br>${safe(c.phone)}<br>${safe(c.address)}</p>`:""}<table><tr><th>Description</th><th>Qty</th><th>Unit Price</th><th>Total</th></tr>${(b.items||[]).map(i=>`<tr><td>${safe(i.desc)}</td><td>${i.qty}</td><td>${money(i.price)}</td><td>${money(i.qty*i.price)}</td></tr>`).join("")}</table>${b.discountAmount>0?`<p style="text-align:right;color:#9a8f80;margin-bottom:2px">Subtotal: ${money(b.subtotal||b.total)}</p><p style="text-align:right;color:#b7791f;margin-bottom:2px">${safe(b.discountLabel||"Discount")}: -${money(b.discountAmount)}</p>`:""}<p class="invoiceTotal">Proposal Total: ${money(b.total)}</p>${b.notes?`<p>${safe(b.notes)}</p>`:""}<p class="small" style="margin-top:16px">This proposal is valid for 30 days from the date above.</p><div class="row noPrint"><button onclick="window.print()">Print or Save PDF</button><button class="secondary" onclick="emailBid('${b.id}')">Email Proposal</button><button class="secondary" onclick="saveInvoiceAsImage()">Save as Image</button><button class="secondary" onclick="textInvoice('${b.customerId}')">Text Proposal</button></div></div>`;
