...
// ── Avatar helpers ─────────────────────────────────────────────────────────────
const AVATAR_COLORS = [
  { bg:"rgba(61,214,140,0.15)",  color:"#3dd68c",  border:"rgba(61,214,140,0.3)"  },
  { bg:"rgba(91,156,246,0.15)",  color:"#5b9cf6",  border:"rgba(91,156,246,0.3)"  },
...
<section id="appScreen" class="hidden">
  ...
  <section id="profitView" class="hidden">
    <div class="box">
      ...
      <p class="small" style="margin-top:10px">Profit is based on money collected minus expenses. Outstanding balances not counted until paid.</p>
    </div>
...
bottomNav.innerHTML = `
  ...
  <button id="navInvoices"  onclick="showView('invoicesView')">${ICONS.invoices}<span>Invoices</span></button>
...
function jobPaidAmount(j)   { const list = jobPayments(j.id); return list.length ? list.reduce((s, p) => s + Number(p.amount || 0), 0) : Number(j.paid || 0); }
...
function customerTotals(cid) {
  const list = jobs.filter(j => j.customerId === cid);
  return { charged: list.reduce((s,j)=>s+Number(j.amount||0),0), paid: list.reduce((s,j)=>s+jobPaidAmount(j),0), owed: list.reduce((s,j)=>s+jobBalance(j),0) };
}

function recurringStatus(r) {
  const diff = Math.ceil((new Date((r.nextDate||today())+"T00:00:00") - new Date(today()+"T00:00:00")) / 86400000);
...
window.saveJob = async function () {
  ...
  const data = { customerId:el("jobCustomer").value, title:el("jobTitle").value.trim(), date:el("jobDate").value||today(), time:el("jobTime").value||"", amount:Number(el("jobAmount").value||0), notes:el("jobNotes").value.trim(), status:existingJob?.status||"Scheduled" };
...
window.markPaid = async function (id) {
  const j = jobs.find(x => x.id === id); if (!j) return;
  const bal = jobBalance(j);
  if (bal <= 0) { alert("This job is already paid."); return; }
...
window.resetRecurringForm = function () {
  ...
  el("recurringCustomer").value=""; el("recurringTitle").value=""; el("recurringNextDate").value=today(); el("recurringTime").value=""; el("recurringAmount").value=""; el("recurringFrequency").value="weekly";
};
...
function updateBidTotal() {
  let total = 0;
  document.querySelectorAll(".bidRow").forEach(row => { total += Number(row.querySelector(".bidQty").value||0) * Number(row.querySelector(".bidPrice").value||0); });
...
window.saveBid = async function () {
  ...
  if (editingBidId) { await updateDoc(doc(db,"bids",editingBidId),data); alert("Bid updated"); }
  else { data.createdAt=new Date().toISOString(); await addDoc(collection(db,"bids"),data); alert("Bid saved"); }
  resetBidForm();
};
...
window.editBid = function (id) {
  ...
  editingBidId = id;
  showView("bidsView"); el("bidFormBox").classList.remove("hidden");
  ...
  el("invoiceArea").innerHTML = `
    <div class="invoice">
      <div class="invoiceTop">
        <div><img class="invoiceLogo" src="logo.png" alt="5Cs Property Services LLC" onerror="this.style.display='none'"><h2>5Cs Property Services LLC</h2><p>Cleaned Up &bull; Fixed Right &bull; Ready To Sell</p></div>
...
window.emailInvoice = function (customerId) {
  ...
  window.location.href = `mailto:${encodeURIComponent(c.email)}?subject=${encodeURIComponent("Invoice from 5Cs Property Services LLC")}&body=${encodeURIComponent(`Hello ${c.name},\n\nBelow is your invoice summary from 5Cs Property Services LLC.\n\nTotal paid: ${money(t.paid)}\nBalance due: ${money(t.owed)}\n\nPayment is due upon receipt.\n\nThank you for your business.\n\n5Cs Property Services LLC`)}`;
};
...
window.copyReminder = function (jobId) {
  ...
  navigator.clipboard.writeText(msg).then(()=>alert("Reminder copied:\n\n"+msg)).catch(()=>alert(msg));
};
...
function renderAll() {
  ...
  const notes = [];
  if (todayJobs.length)    notes.push(`<div class="box" style="background:var(--green-surface);border-color:var(--green-border)"><h3 style="color:var(--green-text)">${todayJobs.length} job${todayJobs.length===1?"":"s"} scheduled today</h3><button class="green" onclick="openTodaySchedule()">View Today</button></div>`);
  if (unpaidJobs.length)   notes.push(`<div class="box" style="background:var(--gold-surface);border-color:var(--gold-border)"><h3 style="color:var(--gold-text)">${unpaidJobs.length} unpaid or partial job${unpaidJobs.length===1?"":"s"}</h3><button class="gold" onclick="openOwedJobs()">Collect Balances</button></div>`);
  if (dueRecurring.length) notes.push(`<div class="box" style="background:var(--blue-surface);border-color:var(--blue-border)"><h3 style="color:var(--blue-text)">${dueRecurring.length} recurring job${dueRecurring.length===1?"":"s"} due soon</h3><button class="blue" onclick="showView('recurringView')">View Recurring</button></div>`);
  el("notificationCenter").innerHTML = notes.length ? notes.join("") : "<p class='small'>No alerts right now.</p>";
  ...
  el("customerList").innerHTML = customers.slice().sort((a,b)=>String(a.name||"").localeCompare(String(b.name||"")))
    ...
    .map(c=>{
      const totals=customerTotals(c.id), phone=cleanPhone(c.phone);
  ...
  el("paymentsList").innerHTML = payments.slice().sort((a,b)=>(b.date||"").localeCompare(a.date||"")).map(p=>{
    const job=jobs.find(j=>j.id===p.jobId);
    return `<div class="box" style="background:var(--s2)"><div style="display:flex;justify-content:space-between;align-items:center"><div><b style="font-size:18px;color:var(--green)">${money(p.amount)}</b><div class="small">${safe(getCustomerName(p.customerId))}</div><div class="small">${dateLabel(p.date)} &bull; ${safe(job?.title||"Payment")}</div>${p.notes?`<div class="small">${safe(p.notes)}</div>`:""}</div><button class="red" style="width:auto;padding:8px 12px;font-size:13px" onclick="deletePayment('${p.id}')">Delete</button></div></div>`;
  }).join("") || "<p class='small'>No payments yet.</p>";

  el("recurringCalendar").innerHTML = recurring.slice().sort((a,b)=>(a.nextDate||"").localeCompare(b.nextDate||"")).map(r=>{
    const s=recurringStatus(r);
    return `<div class="moneyLine"><span style="font-size:13px">${dateLabel(r.nextDate)} ${r.time?timeLabel(r.time):""} &bull; ${safe(r.title)} &bull; ${safe(getCustomerName(r.customerId))}</span><span class="badge ${s.cls}">${s.label}</span></div>`;
  ...
  el("bidsList").innerHTML = bids.length
    ? bids.slice().sort((a,b)=>(b.createdAt||"").localeCompare(a.createdAt||"")).map(b=>`
        <div class="jobCard">
